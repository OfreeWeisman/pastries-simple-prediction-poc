"""
COMPAS-2X Pastries trend analysis – adapted for the compas-2 representation.

The compas-2 dataset uses all-lowercase short ring codes in the `representation`
column (e.g. `ben`, `pyd`, `pyz`) instead of the mixed-case codes used by the
root COMPAS-2X export.  All other columns (ring counts, property values, etc.)
are identical.

Produces the same figures and tables as scripts/analyze_compas_trends.py but
reads from compas-2/data/ and writes to compas-2/data-analysis/.
"""
...
DATA_PATH = ROOT / "compas-2" / "data" / "compas-2x_pastries_features.csv"
OUT_DIR = ROOT / "compas-2" / "data-analysis"
...
# ── Ring code mapping for the compas-2 representation ──────────────────────────
# The compas-2 dataset uses lowercase abbreviated codes in representation strings.
RING_CODE_TO_TYPE = {
    "ben":  "benzene",
    "cbd":  "cyclobutadiene",
    "pyl":  "pyrrole",
...
def savefig(name: str) -> None:
    for ext in ["png", "svg"]:
        plt.savefig(FIG_DIR / f"{name}.{ext}", dpi=220, bbox_inches="tight")
    plt.close()
...
def load_data() -> pd.DataFrame:
    ...
    for column in [
        ...
        *RING_COLUMNS, *HETEROATOM_COLUMNS, *PROPERTY_COLUMNS, "nfod",
    ]:
...
def plot_stacked_property_histograms(
    ...
    plot_colors = colors_by_element(
        component_columns,
        {c: RING_PRIMARY_ELEMENT.get(c, c.upper()) for c in component_columns},
    )
    ...
    for axis, property_column in zip(axes, PROPERTY_COLUMNS):
        table = property_stacked_prevalence(
            frame, property_column, component_columns, bins=22,
        ...
        for component in component_columns:
            ...
            axis.bar(
                x, values, width=width, bottom=bottom,
...
def plot_ring_count_histograms(frame: pd.DataFrame) -> None:
    ...
    for axis, ring in zip(axes, NON_BENZENE_RINGS):
        ...
        axis.bar(
            counts.index.astype(int), counts.values,
            color=MANUAL_COLORS.get(ring, RING_COLORS[ring]),
            edgecolor="white",
        )
...
def plot_indexed_variant_scatter(frame: pd.DataFrame, variant_table: pd.DataFrame) -> None:
    ...
    if rows:
        pd.concat(rows, ignore_index=True).to_csv(TABLE_DIR / "indexed_ring_variant_summary.csv", index=False)
...
def plot_indexed_variant_histograms(frame: pd.DataFrame, variant_table: pd.DataFrame) -> None:
    ...
    for axis, property_column in zip(axes, PROPERTY_COLUMNS):
        ...
        for variant in variants:
            ...
            axis.bar(
                x, values, width=width, bottom=bottom,
                color=colors[variant], edgecolor="white", linewidth=0.2,
    ...
    pd.concat(all_tables, ignore_index=True).to_csv(
        TABLE_DIR / "figure9_like_indexed_variant_stacked_property_histograms.csv", index=False,
    )
...
def write_summary_tables(frame: pd.DataFrame) -> None:
    ...
    pd.DataFrame(ring_summary).sort_values("total_ring_instances", ascending=False).to_csv(
        TABLE_DIR / "ring_type_summary.csv", index=False,
    )
    ...
    frame[[
        "rings", "aromatic_rings", "heteroatoms", "heterocycles", "branch",
        "non_benzene_ring_count", "antiaromatic_ring_count",
        "heterocycle_fraction", "heteroatom_density",
        *PROPERTY_COLUMNS, "nfod",
    ]].describe(percentiles=[0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99]).T.to_csv(
        TABLE_DIR / "dataset_property_structural_summary.csv",
    )

    # Rare indexed variants
    indexed_counts = (
        build_indexed_variant_table(frame)
        .groupby("variant")
        .agg(molecules=("name", "nunique"), variant_label=("variant_label", "first"))
        .reset_index()
    )
    rare = indexed_counts[indexed_counts["molecules"] < 10]
    rare.to_csv(TABLE_DIR / "rare_indexed_ring_variant_molecules_lt10.csv", index=False)
    indexed_counts.to_csv(TABLE_DIR / "indexed_ring_variant_molecule_counts_summary.csv", index=False)
...
def main() -> None:
    ...
    frame = load_data()
    print(f"  {len(frame):,} molecules loaded")

    print("Writing summary tables ...")
    write_summary_tables(frame)

    print("Building indexed variant table ...")
    ...
    print(f"  {len(indexed_variant_table):,} variant rows")

    print("Plotting figure9b stacked histograms ...")
    plot_stacked_property_histograms(
        frame, FIGURE9B_RING_ORDER, RING_LABELS, RING_COLORS,
    ...
    print("Plotting figure7-like heteroatom enrichment ...")
    plot_stacked_property_histograms(
        frame, HETEROATOM_COLUMNS,
    ...
    print("Plotting figure9a HOMO-LUMO scatter ...")
    plot_figure9_scatter(frame)

    print("Plotting antiaromatic distributions ...")
    plot_antiaromatic_property_distributions(frame)

    print("Plotting ring count histograms ...")
    plot_ring_count_histograms(frame)

    print("Plotting heteroatom heatmap ...")
    plot_ring_heteroatom_heatmaps(frame)

    print("Plotting ring-property z-score heatmap ...")
    plot_ring_property_heatmap(frame)

    print("Plotting ring co-occurrence ...")
    plot_ring_cooccurrence(frame)

    print("Plotting indexed variant scatter maps ...")
    plot_indexed_variant_scatter(frame, indexed_variant_table)

    print("Plotting indexed variant stacked histograms ...")
